...
$sqlp=mysqli_query("SELECT programName from ku_program where programID='$program'");
...
<div class="col-md-6 center-margin" style="margin-bottom:30px; ">
    ...
    <table class="table table-striped center-margin">
                    ...
                    <thead>
                     ...
                     <tbody>
                    <?php
	  $sqlSel=mysqli_query("select * from  kankor_student_info where kankorID='$kankorid'") or die(mysqli_error());
	 while($ts=mysqli_fetch_array($sqlSel))
	  {
	  $stdkanid=$ts['kankorStudentID'];
	
	 $sqlSel2=mysqli_query("SELECT programID FROM kankor_student_choice WHERE kankorStudentID='$stdkanid' AND choiceOrder='0' order by kankorStudentID LIMIT 1") or die(mysqli_error());
